# -*- coding: utf-8 -*-
"""
Created on Wed Mar 26 15:08:46 2025
...
for i in range(0,len(graphs_of_undecanes)-1):
   for j in range(i+1,len(graphs_of_undecanes)):
      ...
      vector1.append(round(energy_of_graph(graphs_of_undecanes[i]),5))
      ...
      vector2.append(round(energy_of_graph(graphs_of_undecanes[j]),5))
      topological_indices_undecanes.append(tanimoto_similarity_according_to_condition(vector1,vector2))    

df2=pd.DataFrame(topological_indices_undecanes)
df2.to_excel('topological_indices_undecanes.xlsx',index=False)

with open("topological_indices_undecanes.pkl", "wb") as topological_indicesfile:
    pickle.dump(topological_indices_undecanes, topological_indicesfile)
#
