# -*- coding: utf-8 -*-
"""
Created on Wed Mar 26 15:13:34 2025
...
with open("./data_acquisition_dodecanes/isomer12.pkl", "rb") as file12:
...
#Tanimoto coefficient calculations based on topological indices for dodecanes 
#and exporting the obtained data as pkl and xlsx files.
topological_indices_dodecanes=[]
for i in range(0,len(graphs_of_dodecanes)-1):
   for j in range(i+1,len(graphs_of_dodecanes)):
      vector1=[]
      vector2=[]
      degree_based_indices1=Degree_based_indices(graphs_of_dodecanes[i])
      centrality_measures1=centrality_measures(graphs_of_dodecanes[i])
      energy_and_spectral_radius1=energy_and_spectral_radius_of_graph(graphs_of_dodecanes[i])
      ...
      vector1.append(round(Mostar_Szeged_index(graphs_of_dodecanes[i])[0],5))
      vector1.append(round(Mostar_Szeged_index(graphs_of_dodecanes[i])[1],5))
      vector1.append(round(variance_of_graph(graphs_of_dodecanes[i]),5))
      vector1.append(round(total_irregularity(graphs_of_dodecanes[i]),5))
      vector1.append(round(Wiener_and_Balaban_indices(graphs_of_dodecanes[i])[0],5))
      vector1.append(round(Wiener_and_Balaban_indices(graphs_of_dodecanes[i])[1],5))
      ...
      degree_based_indices2=Degree_based_indices(graphs_of_dodecanes[j])
      centrality_measures2=centrality_measures(graphs_of_dodecanes[j])
      energy_and_spectral_radius2=energy_and_spectral_radius_of_graph(graphs_of_dodecanes[j])
      ...
      vector2.append(round(Mostar_Szeged_index(graphs_of_dodecanes[j])[0],5))
      vector2.append(round(Mostar_Szeged_index(graphs_of_dodecanes[j])[1],5))
      vector2.append(round(variance_of_graph(graphs_of_dodecanes[j]),5))
      vector2.append(round(total_irregularity(graphs_of_dodecanes[j]),5))
      vector2.append(round(Wiener_and_Balaban_indices(graphs_of_dodecanes[j])[0],5))
      vector2.append(round(Wiener_and_Balaban_indices(graphs_of_dodecanes[j])[1],5))
      ...
      topological_indices_dodecanes.append(tanimoto_similarity_according_to_condition(vector1,vector2))   

df2=pd.DataFrame(topological_indices_dodecanes)
df2.to_excel('topological_indices_dodecanes.xlsx',index=False)

with open("topological_indices_dodecanes.pkl", "wb") as topological_indicesfile:
    pickle.dump(topological_indices_dodecanes, topological_indicesfile)
#
