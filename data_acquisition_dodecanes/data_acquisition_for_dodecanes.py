...
def Mostar_Szeged_index(G):
   ...
   for i1 in range(len(Q)):
       ...
       for p in range(len(vertices)):
           #<= VEYA >= olmayacak. Sağlaması C3 kullanılarak yapıldı.
...
for i in range(0,len(graphs_of_dodecanes)-1):
   for j in range(i+1,len(graphs_of_dodecanes)):
      ...
      vector1.append(round(energy_of_graph(graphs_of_dodecanes[i]),5))
      ...
      vector2.append(round(energy_of_graph(graphs_of_dodecanes[j]),5))
